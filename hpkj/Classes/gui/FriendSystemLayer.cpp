...
void FriendSystemLayer::initView()
{
	...
	//}
	pTextUserSex = String::createWithFormat("%s",pConfigMgr->text("t83"))->getCString();
	...
	const char *pTextUserGold = String::createWithFormat
		("%s",pConfigMgr->text("t85"))->getCString();
	...
	const char *pTextUserRecord = String::createWithFormat
		("%s%s%s/%s%s",pConfigMgr->text("t88")," ",pConfigMgr->text("t165")," ",pConfigMgr->text("t166"))->getCString();
	...
	//	("%s%d%%",pConfigMgr->text("t89"),0)->getCString();
	pTextUserWinning = String::createWithFormat
		("%s",pConfigMgr->text("t89"))->getCString();
	...
	//����ID
	float textWidth = 500;
	addFriendID = LabelTTF::create(pConfigMgr->text("t557"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	friendAccount->setFontSize(30);
	friendAccount->setPlaceHolder(pConfigMgr->text("t557"));
...
void FriendSystemLayer::addMyFriendData(DWORD dwUserID)
{
	...
	sqlite3 *DB = handler->getDB();
	sqlite3_stmt *stmt;
	std::sprintf(sql, "select * from FriendInfo where dwUserID=%lu", dwUserID);
...
void FriendSystemLayer::showFriendInfo(int index)
{
	...
	m_pUserName->setString(String::createWithFormat("%s%s",pConfigMgr->text("t82"),szNickName->getCString())->getCString());		
	...
	const char *pTextUserGold = String::createWithFormat
		("%s%d",pConfigMgr->text("t85"),lScore->getValue())->getCString();
	pUserGold->setString(pTextUserGold);

	const char *pTextUserRecord = String::createWithFormat
		("%s%d%s/%d%s",pConfigMgr->text("t88"),lWinCount->getValue(),pConfigMgr->text("t165"),lLostCount->getValue(),pConfigMgr->text("t166"))->getCString();
	...
	{
		pTextUserWinning = String::createWithFormat
			("%s%.2f%%",pConfigMgr->text("t89"),((float)lWinCount->getValue()/(float)lTotalCount->getValue())*100)->getCString();
	}
...
//    bool bRet = isNumber(friendAccount->getText());
	...
	sqlite3 *DB = handler->getDB();
	sqlite3_stmt *stmt;
	std::sprintf(sql, "select * from UserInfo where nickname=\'%s\'", szNickName.c_str());
	int rv = sqlite3_prepare(DB, sql, 200, &stmt, NULL);
	rv = sqlite3_step(stmt);
	if(rv != SQLITE_ROW)
	{
        ...
        std::sprintf(sql, "select * from UserInfo where dwUserID=\'%s\'", szNickName.c_str());
        ...
        if(rv != SQLITE_ROW)
        {
            sqlite3_finalize(stmt);
		PromptBox *promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
		promptBox->setPromptText(pConfigMgr->text("t580"));
	...
	if(existAfter == true){
		if(existBefore == false)
		{
			...
			PromptBox *promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
			promptBox->setPromptText(pConfigMgr->text("t579"));
			this->addChild(promptBox,1);
			bool bRet = handler->isRecordExisted("FriendStatus", userID);
			if(bRet == false)
			{
				memset(sql, 0, sizeof(sql));
				string str = pConfigMgr->text("t581");
				std::sprintf(sql, "insert into FriendStatus(dwUserID,status) values (%lu,\'%s %s\')", userID, szNickName.c_str(),str.c_str());
				handler->execute(sql);
			}
		}else
		{
		PromptBox *promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
		promptBox->setPromptText(pConfigMgr->text("t588"));
		this->addChild(promptBox,1);
		}
	}else{
		PromptBox *promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
