...
bool HelpLayer::init()
{
	...
	{	
		...
		m_pXiaoTiShi = LabelTTF::create(pConfigMgr->text(strTips->getCString()),"HelveticaNeue-Bold", 22);
		...
		LabelTTF *pWanFaShuoMing = LabelTTF::create(pConfigMgr->text("t140"),"",21);
...
void HelpLayer::changeTipsContent(float dt)
{
	...
	m_pXiaoTiShi->setString(pConfigMgr->text(strTips->getCString()));
}
