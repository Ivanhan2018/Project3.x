...
bool ActivityLayer::init()
{
	...
	do 
	{
		...
		activityTips = LabelTTF::create(pConfigMgr->text(String::createWithFormat("t%d",500)->getCString()),"",26);
		...
		for (int i = 0; i <5 ; i++)
		{
			...
			if(i <= 1)
			{
				label1 = LabelTTF::create(pConfigMgr->text(str1->getCString()),"",22,cocos2d::Size(750,0),kCCTextAlignmentLeft);
			}
			else
			{
		...
		//activityTips = LabelTTF::create(pConfigMgr->text(String::createWithFormat("t%d",199)->getCString()),"",26);
		//activityTips->setPosition(Vec2(winSize.width * 0.5,winSize.height * 0.37));
		//activityTips->setColor(ccc3(0,0,0));
		//this->addChild(activityTips,3);
...
void ActivityLayer::changeActivity(int type)
{
	...
	activityTips->setString(pConfigMgr->text(String::createWithFormat("t%d",500+m_nSelectActivity)->getCString()));
