...
TableViewCell *MissionLayer::tableCellAtIndex(TableView *table,ssize_t index)
{
	...
	*/
	...
	float textWidth = 600;
	LabelTTF *title = LabelTTF::create(pConfigMgr->text(String::createWithFormat("t%d", 594+index)->getCString()),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	title->setColor(redColor);
	title->setAnchorPoint(Vec2(0,0.5f));
	title->setPosition(Vec2(120, 52));
	cell->addChild(title, 1);

	LabelTTF *descibeLabel = LabelTTF::create(pConfigMgr->text(String::createWithFormat("t%d", 600+index)->getCString()),"",26,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
...
MissionTableHelper::MissionTableHelper(MissionLayer *fath, int num, MissionLayerTab tag)
{
	...
	sqlite3_stmt *stmt;
	char sql[200]={0};
	std::sprintf(sql, "select * from UserInfo where dwUserID=%ld",pMsgDispatch->m_dwUserID);
...
TableViewCell *MissionTableHelper::tableCellAtIndex(TableView *table,ssize_t index)
{
	...
	float textSize1 = 30;
	float textSize2 = 25;
	LabelTTF *title = LabelTTF::create(pConfigMgr->text(String::createWithFormat("t%d", 700+index)->getCString()),"",textSize1,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	title->setColor(redColor);
	title->setAnchorPoint(Vec2(0,0.5f));
	title->setPosition(Vec2(110, 57));
	cell->addChild(title, 1);

	LabelTTF *descibeLabel = LabelTTF::create(pConfigMgr->text(String::createWithFormat("t%d", 706+index)->getCString()),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	LabelTTF *progress = LabelTTF::create(pConfigMgr->text("t712"),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	{
		if(cbMemberOrder<1){
			descibeLabel1 = LabelTTF::create(pConfigMgr->text("t715"),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
		}else
		{
			if(always < 1){
				descibeLabel1 = LabelTTF::create(pConfigMgr->text("t716"),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
			}else
			{
				descibeLabel1 = LabelTTF::create(pConfigMgr->text("t717"),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
			}
		}
	}else
	{
		if(always < 1){
			descibeLabel1 = LabelTTF::create(pConfigMgr->text("t718"),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
		}else if(always == 1){
			descibeLabel1 = LabelTTF::create(pConfigMgr->text("t716"),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
		}else{
			descibeLabel1 = LabelTTF::create(pConfigMgr->text("t717"),"",textSize2,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
