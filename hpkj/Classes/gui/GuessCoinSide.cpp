...
void GuessCoinSide::initLabel()
{
	...
	betScore = 0;
	float fontSize = 30;
	//LabelTTF *title = LabelTTF::create(pConfigMgr->text("t2001"), "", fontSize);
	...
	fontSize = 25;
	betInfoLabel = LabelTTF::create(pConfigMgr->text("t2006"), "", fontSize);
	...
	fontSize = 30;
	LabelTTF *hintLabel = LabelTTF::create(pConfigMgr->text("t2002"), "", fontSize);
...
void GuessCoinSide::guessCoinRet(Object *pObject)
{
    ...
    //判断正反
    coinSideRet = ((CCBool *)(data->objectAtIndex(0)))->getValue();
    //下注结果
...
{
	betInfoLabel->setString(pConfigMgr->text("t2006"));
	betScore = 0;
	betSide = 0;
...
void GuessCoinSide::pressChip(Object *obj)
{
	...
	if(betScore+chipValue[tag]*10000>score){
		...
		promptBox->setPromptText(pConfigMgr->text("t2007"));
    ...
    if (betScore+chipValue[tag]*10000>10000000) {
        ...
        PromptBox *promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
        promptBox->setPromptText(pConfigMgr->text("t2008"));
       this->addChild(promptBox,1);
        return;
    }
	betScore+=chipValue[tag]*10000;
	betScoreLabel->setString(String::createWithFormat("%d", betScore)->getCString());
	string betInfo = pConfigMgr->text("t2005");
	if(betSide == 1||betSide == 2){
		betInfo = pConfigMgr->text(String::createWithFormat("t%d", betSide+2002)->getCString())+betInfo;
	}
...
void GuessCoinSide::betHead(Object* obj)
{
	...
	betSide = 1;
	string betInfo = pConfigMgr->text("t2003");
	if(betScore!=0)
	{
		betInfo+=pConfigMgr->text("t2005");
	}else{
		betInfo+=pConfigMgr->text("t2006");
	}
	betInfoLabel->setString(betInfo.c_str());
}
...
void GuessCoinSide::betTail(Object* obj)
{
	...
	betSide = 2;
	string betInfo = pConfigMgr->text("t2004");
	if(betScore!=0)
	{
		betInfo+=pConfigMgr->text("t2005");
	}else{
...
void GuessCoinSide::pressConfirm( Object* obj)
{
    ...
    canRotate = false;
    pMsgDispatch->SendPacketWithGuessCoin(betSide==1?true:false, betScore);
//	this->addRandSeed();
//	int num = rand()%2;
}
...
{
	int circle = 4;
	CCOrbitCamera *flipY1 = CCOrbitCamera::create(0.5f, 1, 0, 0, 90, 0, 0);
	CCOrbitCamera *flipY2 = CCOrbitCamera::create(0.5f, 1, 0, 90, 90, 0, 0);
	CCOrbitCamera *flipY3 = CCOrbitCamera::create(0.5f, 1, 0, 180, 90, 0, 0);
	CCOrbitCamera *flipY4 = CCOrbitCamera::create(0.5f, 1, 0, 270, 90, 0, 0);
	Sequence *sequence = Sequence::create(flipY1, CCCallFunc::create(this, callfunc_selector(GuessCoinSide::showTailFrame)),
		flipY2, flipY3, CCCallFunc::create(this, callfunc_selector(GuessCoinSide::showHeadFrame)), flipY4, CCCallFunc::create(this, callfunc_selector(GuessCoinSide::toZero)), NULL);
	CCRepeat *repeat = CCRepeat::create(sequence, circle);
	CCEaseExponentialOut *easeAction; 
	if(tag == 1){
		Sequence *finalSequence = Sequence::create(repeat, flipY1, CCCallFunc::create(this, callfunc_selector(GuessCoinSide::showTailFrame)),
			flipY2, NULL);
		easeAction= CCEaseExponentialOut::create(dynamic_cast<CCActionInterval *>(finalSequence));
	}else{
		easeAction= CCEaseExponentialOut::create(dynamic_cast<CCActionInterval *>(repeat));
	}
...
void GuessCoinSide::coinRotateEnd(Node *obj)
{
    ...
    if (tempScore>0) {
        str = String::createWithFormat("%s %lld %s", pConfigMgr->text("t2045"), tempScore, pConfigMgr->text("t2046"));
    }else{
        str = String::createWithFormat("%s %lld %s", pConfigMgr->text("t2047"), tempScore, pConfigMgr->text("t2048"));
    }
    PromptBox *promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
