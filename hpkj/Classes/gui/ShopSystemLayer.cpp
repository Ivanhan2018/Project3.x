...
void ShopSystemLayer::initView()
{
	...
	std::sprintf(info, "%I64d", score);
#else
	std::sprintf(info, "%lld", score);
#endif
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
	std::sprintf(info, "%I64d", bankScore);
#else
	std::sprintf(info, "%lld", bankScore);
#endif
	...
	//������ǩ
	withdrawNumLabel = LabelTTF::create(pConfigMgr->text("t569"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	withdrawNum->setFontSize(30);
	this->addChild(withdrawNum,1);

	//�����ǩ
	withdrawPasswordLabel = LabelTTF::create(pConfigMgr->text("t570"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	withdrawPassword->setFontSize(30);
	...
	//������ǩ
	depositNumLabel = LabelTTF::create(pConfigMgr->text("t571"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	depositNum->setFontSize(30);
	this->addChild(depositNum,1);

	//�����ǩ
	depositPasswordLabel = LabelTTF::create(pConfigMgr->text("t570"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	depositPassword->setFontSize(30);
	...
	//ת���ǳƱ�ǩ
	transferNickname = LabelTTF::create(pConfigMgr->text("t573"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	//������ǩ
	transferNumLabel = LabelTTF::create(pConfigMgr->text("t574"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	transferNum->setFontSize(30);
	this->addChild(transferNum,1);

	//�����ǩ
	transferPasswordLabel = LabelTTF::create(pConfigMgr->text("t570"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
	...
	transferPassword->setFontSize(30);
...
{
    score = pMsgDispatch->m_dwlUserScore;
    bankScore = pMsgDispatch->m_dwlUserInsure;
    char str[100];
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
    std::sprintf(str, "%I64d", score);
#else
    std::sprintf(str, "%lld", score);
#endif
    scoreLabel->setString(str);
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
    std::sprintf(str, "%I64d", bankScore);
#else
    std::sprintf(str, "%lld", bankScore);
#endif
    bankScoreLabel->setString(str);
    if(promptBox!=NULL)
    {
        promptBox->setPromptText(pMsgDispatch->szDescribeString);
        
        char message[1024];
        time_t tt = time(NULL);//��䷵�ص�ֻ��һ��ʱ��cuo
        tm* t= localtime(&tt);
        std::sprintf(message, "insert into PersonalMsg(msg) values(\'%s     %d-%02d-%02d %02d:%02d:%02d\')", pMsgDispatch->szDescribeString,
...
{
	pMsgDispatch->SendPacketWithQueryBankInfo();
}
...
void ShopSystemLayer::buyButton(Object *obj)
{
	...
	if(objTag >= BuyButton_Article+1 && objTag <= BuyButton_Article + 9)
	{
        ...
        buyArticleType2->setString(pConfigMgr->text(str->getCString()));
	}
...
void ShopSystemLayer::pressOk(Object *obj)
{
	if(buyViewVisible == true){
		...
		{
			pMsgDispatch->SendPacketWithDeposit(inputScore);
			...
			promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
			promptBox->setPromptText(pConfigMgr->text("t577"));
			this->addChild(promptBox,2);
		}
		this->setDepositViewVisible(false);
		...
		if(inputScore<=bankScore||true)
		{
			...
			pMsgDispatch->SendPacketWithWithdraw(inputScore, passStr);
			promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
			promptBox->setPromptText(pConfigMgr->text("t556"));
			this->addChild(promptBox,2);
		}else{
		...
		if(inputScore<=bankScore||true)
		{
			...
			pMsgDispatch->SendPacketWithSendScore(nicknameStr, inputScore, passStr, 0);
			promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
			promptBox->setPromptText(pConfigMgr->text("t556"));
			this->addChild(promptBox,2);
		}else{
    ...
    if(currentItem == TabLayerBaseTag+1)
    {
        ...
        if (num<=0) {
            promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
            promptBox->setPromptText(pConfigMgr->text("t2030"));
           this->addChild(promptBox,2);
        }else{
            pMsgDispatch->SendPacketWithBuyGoods(objTag-200+2, num);
            promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
            promptBox->setPromptText(pConfigMgr->text("t556"));
           this->addChild(promptBox,2);
        }
...
TableViewCell *ShopSystemLayer::tableCellAtIndex(TableView *table,ssize_t index)
{
	...
	{
		std::sprintf(name, "%s%d.png", ShopCoinTableFileName, (int)index*3+i);
		...
		std::sprintf(name, "%s%d_1.png", BuyCoinButtonFileName, (int)index*3+i);
		Sprite * buyNormalSprite = Sprite::createWithSpriteFrame(spriteFrame(name));
		memset(name, 0, sizeof(name));
		std::sprintf(name, "%s%d_2.png", BuyCoinButtonFileName, (int)index*3+i);
...
TableViewCell *ArticleTableHelper::tableCellAtIndex(TableView *table,ssize_t index)
{
	...
	{
		std::sprintf(name, "%s%d.png", ShopArticleTableFileName, (int)index*3+i);
		Sprite *cellBk = Sprite::createWithSpriteFrame(spriteFrame(name));
		cellBk->setPosition(Vec2(startX, startY));
		cell->addChild(cellBk);
