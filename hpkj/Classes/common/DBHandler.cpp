...
bool DBHandler::isRecordExisted(char *name, int ID)
{
	...
	bool isExisted_; 
	char tempStr[100];
	std::sprintf(tempStr, "select count(*) from %s where dwUserID = %d", name, ID);
...
bool DBHandler::isRecordExisted(char *tableName, char *fieldName, char *fieldValue, bool isText)
{
	...
	char tempStr[100];
	if(isText == true){
		std::sprintf(tempStr, "select count(*) from %s where %s = \'%s\'", tableName, fieldName, fieldValue);
	}else{
		std::sprintf(tempStr, "select count(*) from %s where %s = %s", tableName, fieldName, fieldValue);
	}
	sqlite3_exec( pDB, tempStr , isExisted, &isExisted_, &errMsg ); 
...
int  DBHandler::updateValueInfo(int levelID, int userID, float time, int star, int money, int score)
{
	...
	bool isExisted_; 
	char tempStr[100];
	std::sprintf(tempStr, "select count(*) from levelInfo where (levelID=%d and userID=%d)", levelID, userID);
	sqlite3_exec( pDB, tempStr , isExisted, &isExisted_, &errMsg ); 
	if(!isExisted_)
	{
		char sql[100];
		std::sprintf(sql, "insert into levelInfo(levelID,userID,time,star,money,score)values(%d,%d,%f,%d,%d,%d)", levelID, userID, time, star, money, score);
	...
	}else
	{
		...
		sqlite3_stmt *stmt;
		float timeRecord;
		std::sprintf(sql, "select time from levelInfo where (levelID=%d and userID=%d)", levelID, userID);
		...
		{	
			char sqlChar[150];
			std::sprintf(sqlChar, "update levelInfo set levelID=%d,userID=%d,time=%f,star=%d,money=%d,score=%d where(levelID=%d and userID=%d)", levelID, userID, time, star, money, score, levelID, userID);
...
void DBHandler::updateUserInfo(int userID)
{
	...
	sqlite3_stmt *stmt;
	int score;
	std::sprintf(sql, "select sum(score) from levelInfo where userID=%d", userID);
	int rv = sqlite3_prepare(pDB, sql, 100, &stmt, NULL);
	rv = sqlite3_step(stmt);
	if(rv == SQLITE_ROW)
	{
	...
	{	
		char sqlChar[150];
		std::sprintf(sqlChar, "update userInfo set totalMoney=%d,totalScore=%d where(ID=%d)", totalMoney, score, userID);
		rv = this->execute(sqlChar);
	}else
	{
		char sqlChar[150];
		std::sprintf(sqlChar, "update userInfo set totalMoney=%d where(ID=%d)", totalMoney, userID);
		rv = this->execute(sqlChar);
	}
