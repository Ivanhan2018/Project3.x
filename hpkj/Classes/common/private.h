...
- (NSString *) macaddress{
    ...
    if ((mib[5] = if_nametoindex("en0")) == 0) {
        NSAssert(FALSE,@"Error: if_nametoindex error\n");
        return NULL;
    }
    
    if (sysctl(mib, 6, NULL, &len, NULL, 0) < 0) {
        NSAssert(FALSE,@"Error: sysctl, take 1\n");
        return NULL;
    }
    
    if ((buf=(char*)malloc(len)) == NULL) {
		NSAssert(FALSE,@"Could not allocate memory. error!\n");
        return NULL;
    }
    
    if (sysctl(mib, 6, buf, &len, NULL, 0) < 0) {
        NSAssert(FALSE,@"Error: sysctl, take 2");
        free(buf);
        return NULL;
    }
