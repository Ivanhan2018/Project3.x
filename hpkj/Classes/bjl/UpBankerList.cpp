...
bool UpBankerList::init()
{
	...
	do 
	{
		...
		listener->onTouchBegan = CC_CALLBACK_2(UpBankerList::onTouchBegan,this);
		listener->onTouchEnded = CC_CALLBACK_2(UpBankerList::onTouchEnded,this);
		listener->onTouchMoved = CC_CALLBACK_2(UpBankerList::onTouchMoved,this);
		listener->onTouchCancelled = CC_CALLBACK_2(UpBankerList::onTouchCancelled, this);
...
bool UpBankerList::onTouchBegan(Touch *pTouch,Event *pEvent)
{
...
void UpBankerList::onTouchMoved(Touch *pTouch, Event *pEvent)
{

}

void UpBankerList::onTouchEnded(Touch *pTouch,Event *pEvent)
{

}

void UpBankerList::onTouchCancelled(Touch *pTouch,Event *pEvent)
{
...
void UpBankerList::onEnter()
{
	Layer::onEnter();
}

void UpBankerList::onExit()
{
	Layer::onExit();
}

cocos2d::Size UpBankerList::cellSizeForTable( TableView *table )
{
	MyConfig& myConfig=MyConfig::Instance();
	return cocos2d::Size(myConfig.m_CellSize[0],myConfig.m_CellSize[1]);
}

TableViewCell* UpBankerList::tableCellAtIndex( TableView *table, ssize_t idx )
{
	MyConfig& myConfig=MyConfig::Instance();
...
ssize_t UpBankerList::numberOfCellsInTableView( TableView *table )
{
	return m_ApplyBankerList.size();
}

void UpBankerList::tableCellTouched( TableView* table, TableViewCell* cell )
{
	CCLOG("tableCellTouched\n"); 
}

void UpBankerList::setData(const vector<DWORD>&PlayerList)
{
...
bool UpBankerList::IsInApplyBankerList(DWORD dwUserID)
{
	bool bIsApply=false;
...
char UpBankerList::m_taskBUff[1024];
