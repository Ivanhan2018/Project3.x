...
public:
	...
	tagGameServer getGameServerByIndex(int index);			//获取第几个房间		
private:
...
public:
	...
	~CGlobalUnits();
private:
	tagGlobalUserData*   m_pGloblaUserData;
...
public:
	...
	static string getUserFace(unsigned int faceID,unsigned int sex)
	{
		...
		{
			std::sprintf(buf, "dtgb_touxiangnan%d.png.png",faceID + 1);
			return  buf;
		}

		faceID -= 20;

		std::sprintf(buf, "dtgb_touxiangnv%d.png",faceID % 8 + 1);
		return buf;
	}
