...
void HNMJPlayer::showHandCard(CMD_WeaveItem* pWeave,int iWeaveCout,BYTE* pHandCard,int iHandCout)
{
	...
	if (iWeaveCout*3+iHandCout > MAX_COUNT)
	{
		pCardNode->setScale(0.9f);
	}
								 ...
								 ,BYTE* pChiHuCard)
{
	...
	for (int i = 0;i < 2 ;i++)
	{
		...
		std::string kImagic = WidgetFun::getWidgetUserInfo(pNode,"MingImagic");
		setCardImagic(pNode,pChiHuCard[i],kImagic);
	}
}
...
void HNMJPlayer::showCSGangCard(int nCard1,int nCard2,bool bOut)
{
	...
	if (bOut)
	{
		...
		WidgetFun::runWidgetAction(pHandOutCard1,"OutAni");//等待2秒显示
		WidgetFun::runWidgetAction(pHandOutCard2,"OutAni");//等待2秒显示
