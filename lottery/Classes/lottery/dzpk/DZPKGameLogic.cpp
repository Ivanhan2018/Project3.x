#include <stdlib.h>
#include "DZPKGameLogic.h"

//////////////////////////////////////////////////////////////////////////

//扑克数据
BYTE DZPKGameLogic::m_cbCardData[DZPK_FULL_COUNT]=
{
...
//混乱扑克
void DZPKGameLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
//#ifndef WIN32 
//	return ;
//#else
...
//获取类型
BYTE DZPKGameLogic::GetCardType(BYTE cbCardData[], BYTE cbCardCount)
{
	//数据校验
	...
	//扑克分析
	tagAnalyseResult_dzpk AnalyseResult;
...
//最大牌型
BYTE DZPKGameLogic::FiveFromSeven(BYTE cbHandCardData[],BYTE cbHandCardCount,BYTE cbCenterCardData[],BYTE cbCenterCardCount,BYTE cbLastCardData[],BYTE cbLastCardCount)
{
//#ifndef WIN32 
//	return 0;
//#else
	//临时变量
	BYTE cbTempCardData[DZPK_HAND_CARD_COUNT+MAX_CENTERCOUNT],cbTempLastCardData[5];
	memset(cbTempCardData,0,sizeof(cbTempCardData));
	memset(cbTempLastCardData,0,sizeof(cbTempLastCardData));

	//拷贝数据
	memcpy(cbTempCardData,cbHandCardData,sizeof(BYTE)*DZPK_HAND_CARD_COUNT);
...
//查找最大
bool DZPKGameLogic::SelectMaxUser(BYTE bCardData[DZPKGAME_PLAYER][MAX_CENTERCOUNT],UserWinList &EndResult,const LONGLONG lAddScore[])
{
//#ifndef WIN32 
//	return 0;
//#else
	...
	//过滤全零
	if(i==DZPKGAME_PLAYER)return false;

	//查找最大用户
	WORD wTemp = wWinnerID;
	for(WORD i=1;i<DZPKGAME_PLAYER;i++)
	{
		WORD j=(i+wTemp)%DZPKGAME_PLAYER;
	...
	for(WORD i=0;i<DZPKGAME_PLAYER;i++)
	{
...
//排列扑克
void DZPKGameLogic::SortCardList(BYTE cbCardData[], BYTE cbCardCount)
{
	//转换数值
	BYTE cbLogicValue[DZPK_FULL_COUNT];
...
//查找扑克
BYTE DZPKGameLogic::GetSameCard(const BYTE bCardData[],const BYTE bMaxCard[],BYTE bCardCount,BYTE bMaxCardCount,BYTE bResultData[])
{
...
//逻辑数值
BYTE DZPKGameLogic::GetCardLogicValue(BYTE cbCardData)
{
	//扑克属性
...
//对比扑克
BYTE DZPKGameLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount)
{
//#ifndef WIN32 
//	return 0;
//#else
	...
	case CT_HU_LU:			//葫芦
		{
			//分析扑克
			tagAnalyseResult_dzpk AnalyseResultNext;
			tagAnalyseResult_dzpk AnalyseResultFirst;
...
//分析扑克
void DZPKGameLogic::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagAnalyseResult_dzpk & AnalyseResult)
{
//#ifndef WIN32 
//	return;
//#else
