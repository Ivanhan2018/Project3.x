...
Scene* LotteryRecordSubRecharge::scene()
{
	Scene *scene = Scene::create();
	LotteryRecordSubRecharge *layer = LotteryRecordSubRecharge::create();
	scene->addChild(layer);
	return scene;
}

LotteryRecordSubRecharge::LotteryRecordSubRecharge()
{
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryRecordSubRecharge::getXJCZLogCountRet), "GetXJCZLogCountRet", NULL);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryRecordSubRecharge::getXJCHZHLogRet), "GetXJCZLogRet", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryRecordSubRecharge::refreshLayer), "Refresh_Data_Recon", NULL);

}

LotteryRecordSubRecharge::~LotteryRecordSubRecharge()
{
    NotificationCenter::getInstance()->removeObserver(this, "GetXJCZLogCountRet");
    NotificationCenter::getInstance()->removeObserver(this, "GetXJCZLogRet");
	NotificationCenter::getInstance()->removeObserver(this, "Refresh_Data_Recon");

}

void LotteryRecordSubRecharge::getXJCZLogCountRet(Object *obj)
{
...
void LotteryRecordSubRecharge::getXJCHZHLogRet(Object *obj)
{
...
bool LotteryRecordSubRecharge::init()
{
...
void LotteryRecordSubRecharge::initView()
{
    setTitle(896);
    ...
    MenuItemImage *confirmItem = MenuItemImage::create("beting22.png", "beting23.png", CC_CALLBACK_1(LotteryRecordSubRecharge::pressConfirm,this));
    ...
    gap1->setPosition(Vec2(0, startY-gapY*2 -1));
    gap1->setScaleX(SCREEN_WIDTH / gap1->getContentSize().width);
    this->addChild(gap1,3);
  ...
  s_t_start = BetLimit::AjustDateStr(todayStr);
	...
	EntityMgr::instance()->getDispatch()->SendPacketWithGetXjChzhLogCount(true, s_t_start.c_str(), s_t_end.c_str(), n_t_type, userID, s_t_account.c_str());
}

void LotteryRecordSubRecharge::tableCellTouched(TableView* table, TableViewCell* cell)
{
	int cellTag = cell->getTag();
}

cocos2d::Size LotteryRecordSubRecharge::cellSizeForTable(TableView *table)
{
	cocos2d::Size size;
...
TableViewCell* LotteryRecordSubRecharge::tableCellAtIndex(TableView *table, ssize_t idx)
{
	...
	for(int i = 0 ; i < 4 ; i++){
		String *labelStr = String::create(ConfigMgr::instance()->text("display_text.xml",
			String::createWithFormat("t%d", 918+i*2)->getCString()));
		LabelTTF *label = LabelTTF::create(labelStr->getCString(), "", fontSize);
		label->setAnchorPoint(Vec2(1,0.5f));
		...
		String *labelStr1 = String::create(ConfigMgr::instance()->text("display_text.xml",
			String::createWithFormat("t%d", 919+i*2)->getCString()));
...
ssize_t LotteryRecordSubRecharge::numberOfCellsInTableView(TableView *table)
{
	if(recordData->count() == 0){
		return 1;
	}
...
void LotteryRecordSubRecharge::scrollViewDidScroll(ScrollView* view)
{

}

void LotteryRecordSubRecharge::scrollViewDidZoom(ScrollView* view)
{

}

void LotteryRecordSubRecharge::pressConfirm(Object *obj)
{
	playButtonSound();
...
void LotteryRecordSubRecharge::sendPacket()
{   
	if (checkIsQuerying()) return;LotteryRecord::sendPacket();
	EntityMgr::instance()->getDispatch()->SendPacketWithGetXJCHZHLog(currentPage, pageSize, true, s_t_start.c_str(),s_t_end.c_str(), n_t_type, userID, s_t_account.c_str());
}

//根据彩种ID和玩法ID，返回玩法名称
const char *LotteryRecordSubRecharge::getGameKindTitle(int gameType, int gameKind)
{
	return BetLimit::GetWanFaNameFromId(gameType, gameKind).c_str();
}

void LotteryRecordSubRecharge::refreshLayer(Object *obj)
{	
	isQuerying = false;
	sendPacket();
}