#include "RoomLayer.h"
#include "bjpk10KindScrollView.h"
...
bjpk10KindScrollView::bjpk10KindScrollView()
{
...
Scene* bjpk10KindScrollView::scene()
{
	Scene * scene = NULL;
	do 
	{
		scene = Scene::create();
		CC_BREAK_IF(! scene);
		bjpk10KindScrollView *layer = bjpk10KindScrollView::create();
		CC_BREAK_IF(! layer);
		scene->addChild(layer);
	} while (0);
	return scene;
}
bool bjpk10KindScrollView::init()
{
	bool bRet = false;
	do 
	{
...
void bjpk10KindScrollView::initData()
{
	//界面数据
	lyWanFa = nullptr;
	m_node = nullptr;
...
void bjpk10KindScrollView::initView()
{
	auto listener = EventListenerTouchOneByOne::create();
	listener->setSwallowTouches(true);
	listener->onTouchBegan = CC_CALLBACK_2(bjpk10KindScrollView::onTouchBegan, this);	
	listener->onTouchEnded = CC_CALLBACK_2(bjpk10KindScrollView::onTouchEnded, this);
	listener->onTouchMoved = CC_CALLBACK_2(bjpk10KindScrollView::onTouchMoved, this);
	listener->onTouchCancelled = CC_CALLBACK_2(bjpk10KindScrollView::onTouchCancelled, this);
	...
	int chongQingKindCount = atoi(ConfigMgr::instance()->text("display_pk10.xml", "t1000"));
	int iViewHeight = 0;
	...
	{
		//位置
		int iPosTitle = atoi(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", 1000+i + 1)->getCString()));
		//分类名称 文字
		LabelTTF *kindTitleLabel = LabelTTF::create(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", iPosTitle)->getCString()), "", fontSize);
		...
		//大分类的个数
		int cataCount = atoi(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", iPosTitle + 1)->getCString()));

		for(int cata_i = 0;cata_i < cataCount;cata_i ++)
		{
			int iPosCata = atoi(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", iPosTitle + cata_i + 2)->getCString()));

			//显示前面 分类名称
			std::string strTitle = ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", iPosCata)->getCString());
			...
			//菜单叶子 位置
			int itemPos = atoi(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", iPosCata + 1)->getCString()));

			//叶子个数
			int itemCount = atoi(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", itemPos)->getCString()));

			for(int j = 0 ; j < itemCount ; j++){

				String* kindStr = String::create(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d", itemPos + j + 1)->getCString()));
				...
				if(kindItemTag == selectedSpriteTag){
					...
					//文字
					LabelTTF *kindItem = LabelTTF::create(ConfigMgr::instance()->text("display_pk10.xml",kindStr->getCString()), "", fontSize);
					kindItem->setAnchorPoint(Vec2::ANCHOR_MIDDLE);	
					kindItem->setTag(9527);
					kindItem->setPosition(sptKindItem->getContentSize() / 2);					
...
void bjpk10KindScrollView::pressKindButton(int tag)
{
	playButtonSound();
...
void bjpk10KindScrollView::onEnter()
{
	Layer::onEnter();
	//
...
void bjpk10KindScrollView::onExit()
{
	//
	Layer::onExit();
}

bool bjpk10KindScrollView::onTouchBegan(Touch *pTouch, Event *pEvent)
{
...
void bjpk10KindScrollView::onTouchMoved(Touch *pTouch, Event *pEvent)
{
...
void bjpk10KindScrollView::onTouchEnded(Touch *pTouch, Event *pEvent)
{
	selectedSprite->setVisible(false);
...
void bjpk10KindScrollView::onTouchCancelled(Touch *pTouch, Event *pEvent)
{

}

void bjpk10KindScrollView::refreshContent()
{
	...
	for(int i = 0;i < LIST_MAX_SIZE;i ++)
	{
		...
		if(!isOutList)
		{
			ltfWord->setString(ConfigMgr::instance()->text("display_pk10.xml",String::createWithFormat("t%d",20 + PK10WanfaMap[MyBetData::getInstance()->getWanfaAt(i)])->getCString()));
			if(m_isNowDel)
			{
...
void bjpk10KindScrollView::refreshMenu()
{
