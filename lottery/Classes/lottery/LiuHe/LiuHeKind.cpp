...
Scene* LiuHeKind::scene()
{
	...
	do 
	{
		scene = Scene::create();
		CC_BREAK_IF(! scene);
		LiuHeKind *layer = LiuHeKind::create();
		CC_BREAK_IF(! layer);
...
void LiuHeKind::initView()
{

	auto listener = EventListenerTouchOneByOne::create();
	listener->setSwallowTouches(true);
	listener->onTouchBegan = CC_CALLBACK_2(LiuHeKind::onTouchBegan, this);	
	listener->onTouchEnded = CC_CALLBACK_2(LiuHeKind::onTouchEnded, this);
	listener->onTouchMoved = CC_CALLBACK_2(LiuHeKind::onTouchMoved, this);
	listener->onTouchCancelled = CC_CALLBACK_2(LiuHeKind::onTouchCancelled, this);
	...
	int chongQingKindCount = atoi(ConfigMgr::instance()->text("display_6he.xml", "t1000"));
	int iViewHeight = 0;
	...
	{
		//位置
		int iPosTitle = atoi(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", 1000+i + 1)->getCString()));
		//分类名称 文字
		LabelTTF *kindTitleLabel = LabelTTF::create(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", iPosTitle)->getCString()), "", fontSize);
		...
		//大分类的个数
		int cataCount = atoi(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", iPosTitle + 1)->getCString()));

		for(int cata_i = 0;cata_i < cataCount;cata_i ++)
		{
			int iPosCata = atoi(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", iPosTitle + cata_i + 2)->getCString()));

			//显示前面 分类名称
			std::string strTitle = ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", iPosCata)->getCString());
			...
			//菜单叶子 位置
			int itemPos = atoi(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", iPosCata + 1)->getCString()));

			//叶子个数
			int itemCount = atoi(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", itemPos)->getCString()));

			for(int j = 0 ; j < itemCount ; j++){

				String* kindStr = String::create(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d", itemPos + j + 1)->getCString()));
				...
				if(kindItemTag == selectedSpriteTag){
					...
					//文字
					LabelTTF *kindItem = LabelTTF::create(ConfigMgr::instance()->text("display_6he.xml",kindStr->getCString()), "", fontSize);
					kindItem->setAnchorPoint(Vec2::ANCHOR_MIDDLE);	
					kindItem->setTag(9527);
					kindItem->setPosition(sptKindItem->getContentSize() / 2);					
...
void LiuHeKind::pressKindButton(int tag)
{
	playButtonSound();
...
void LiuHeKind::onExit()
{
	//
	Layer::onExit();
}

bool LiuHeKind::onTouchBegan(Touch *pTouch, Event *pEvent)
{
...
void LiuHeKind::onTouchMoved(Touch *pTouch, Event *pEvent)
{
...
void LiuHeKind::onTouchEnded(Touch *pTouch, Event *pEvent)
{
	selectedSprite->setVisible(false);
...
void LiuHeKind::onTouchCancelled(Touch *pTouch, Event *pEvent)
{

}

void LiuHeKind::refreshContent()
{
	...
	for(int i = 0;i < LIST_MAX_SIZE;i ++)
	{
		...
		if(!isOutList)
		{
			ltfWord->setString(ConfigMgr::instance()->text("display_6he.xml",String::createWithFormat("t%d",20 + tagLiuHe[MyBetData::getInstance()->getWanfaAt(i)])->getCString()));
			if(m_isNowDel)
			{
