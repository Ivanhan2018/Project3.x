...
Scene* LotteryGambleLayer::scene()
{
...
LotteryGambleLayer::LotteryGambleLayer()
{
	...
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::GetLastYueRet), "GetLastYueRet", NULL);  ///余额查询
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::ShiYiXuanWoNumber), "ShiYiXuanWoNumber", NULL);  ///追号所花费的金额
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::ZhuiHaoTouZhuJinENumber), "ZhuiHaoTouZhuJinENumber", NULL);  ///追号所花费的金额
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::queryGameRet), "QueryGameRet", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::touZhuRet), "CQSSCRet", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::changeKind), "CQSSCChangeKind", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::deleteItem), "DeleteItem", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::addRandomItem), "AddRandomItem", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::cartConfirm), "CartConfirm", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::clearNumbers), "ClearNumbers", NULL);
	//NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::getSystemTimeObject), "GetSystemTimeRet", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::updateTouZhuJine), "UpdateTouZhuJine", NULL);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::KindMyFanDianRet), "QueryMyFanDianRet", NULL);

	//追号验证
	NotificationCenter::getInstance()->addObserver(this,callfuncO_selector(LotteryGambleLayer::isFenDanHour), "IsFendanHour", NULL);

	//跨年期号处理
	NotificationCenter::getInstance()->addObserver(this,callfuncO_selector(LotteryGambleLayer::kuaNianExpect), "KuaNian", NULL);

	//结果查询
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(LotteryGambleLayer::queryGameMoreRet), "QueryMoreGameRet", NULL);
}

void LotteryGambleLayer::getSystemTime(float dt)
{
	//跨年
...
LotteryGambleLayer::~LotteryGambleLayer()
{
	...
	this->unschedule(schedule_selector(LotteryGambleLayer::updatePeriod));
	this->unschedule(schedule_selector(LotteryGambleLayer::getSystemTime));
	this->unschedule(schedule_selector(LotteryGambleLayer::myUpdate));
...
void LotteryGambleLayer::setGameKind(CaiZhong kind)
{
	//初始化数据
	MyBetData::getInstance()->setCurCaiZhongId(kind);

	int last = MyBetData::getInstance()->getLastWanfaId();
	if(last == -1) last = tagQiXingCai[1];

	this->setWanFaKind((QiXingCaiKind)last);
	selectedItemTag = 1;

	for(int i = 0;i < MyBetData::getInstance()->getWanfaSize();i ++)
	{
		if(tagQiXingCai[MyBetData::getInstance()->getWanfaAt(i)] == last)
		{
	...
	this->schedule(schedule_selector(LotteryGambleLayer::updatePeriod), 3);

	//跨年处理
#if 1
	EntityMgr::instance()->getDispatch()->SendPacketWithGetKuaNianQiHao(gameKind);
	this->schedule(schedule_selector(LotteryGambleLayer::getSystemTime), 3);
#endif
	this->schedule(schedule_selector(LotteryGambleLayer::myUpdate), 1);
	...
	EntityMgr::instance()->getDispatch()->SendPacketWithTouZhuCQSSC_FanDian(gameKind, BetLimit::GetWanfaIdFromId(wanFaKind,4));
...
void LotteryGambleLayer::updatePeriod(float dt)
{
	EntityMgr::instance()->getDispatch()->SendPacketQueryGameResult(gameKind);
}

void LotteryGambleLayer::refreshLayer()
{
	//更新期号
	EntityMgr::instance()->getDispatch()->SendPacketQueryGameResult(gameKind);
	//更新余额
...
void LotteryGambleLayer::setWanFaKind(QiXingCaiKind kind)
{
	wanFaKind = kind;
...
void LotteryGambleLayer::myUpdate(float dt)
{
...
//初始化默认数据
//确定进入以后默认玩法
void LotteryGambleLayer::initData()
{
	...
	selectedItemTag = 1;
	wanFaKind = QiXingCai_LiangDing; //默认七星彩两定
...
//初始化界面
void LotteryGambleLayer::initView()
{
	...
	MenuItemSprite *pCloseItemSprite = MenuItemSprite::create
		(pCloseNormalButton,pCloseSelectButton,CC_CALLBACK_1(LotteryGambleLayer::back,this));
	...
	MenuItemSprite *setupItem = MenuItemSprite::create(setupItemNormalImage,setupItemSelectedImage,CC_CALLBACK_1(LotteryGambleLayer::setting,this));
	...
	//MenuItemLabel *lbItem = MenuItemLabel::create(moreItemLabel,CC_CALLBACK_1(LotteryGambleLayer::jumpResult,this));
	...
	MenuItemSprite *deleteItem = MenuItemSprite::create(deleteNormalImage,deleteSelectedImage,CC_CALLBACK_1(LotteryGambleLayer::toDelete,this));
	MenuItemSprite *randomGambleItem = MenuItemSprite::create(randomNormalImage,randomSelectedImage,CC_CALLBACK_1(LotteryGambleLayer::toRandomGamble,this));
	...
	moreItem = MenuItemSprite::create(moreNormalImage2,moreSelectedImage2,CC_CALLBACK_1(LotteryGambleLayer::toMore,this));
	...
	MenuItemSprite *beiAddItem = MenuItemSprite::create(beiAdd,beiAddPress,CC_CALLBACK_1(LotteryGambleLayer::beiAdd,this));
	MenuItemSprite *beiMinusItem = MenuItemSprite::create(beiMinus,beiMinusPress,CC_CALLBACK_1(LotteryGambleLayer::beiMinus,this));
	...
	MenuItemSprite *FanDianSelectedImage = MenuItemSprite::create(FanDianSprite_1,FanDianSprite_2,CC_CALLBACK_1(LotteryGambleLayer::FanDianMinus,this));
	...
	cartItem = MenuItemSprite::create(cartNormalImage,cartSelectedImage,CC_CALLBACK_1(LotteryGambleLayer::toGamble,this));
	MenuItemSprite *confirmItem = MenuItemSprite::create(confirmNormalImage,confirmSelectedImage,CC_CALLBACK_1(LotteryGambleLayer::toCart,this));
	...
	YuanSelectedImageLayer = MenuItemSprite::create(YuanSprite_1,YuanSprite_2,YuanSprite_3,CC_CALLBACK_1(LotteryGambleLayer::XuanZeMonenyLayer,this));
	YuanSelectedImageLayer->setTag(YuanShiShiCAITag);
	JiaoSelectedImageLayer = MenuItemSprite::create(JiaoSprite_1,JiaoSprite_2,JiaoSprite_3,CC_CALLBACK_1(LotteryGambleLayer::XuanZeMonenyLayer,this));
	JiaoSelectedImageLayer->setTag(JiaoShiShiCAITag);
	FenSelectedImageLayer = MenuItemSprite::create(FenSprite_1,FenSprite_2,FenSprite_3,CC_CALLBACK_1(LotteryGambleLayer::XuanZeMonenyLayer,this));
	FenSelectedImageLayer->setTag(FenShiShiCAITag);
	//新增厘模式
	LiSelectedImageLayer = MenuItemSprite::create(LiSprite_1, LiSprite_2, LiSprite_3,CC_CALLBACK_1(LotteryGambleLayer::XuanZeMonenyLayer, this));
...
void LotteryGambleLayer::queryGameRet(Object *obj)
{
	...
	for (int i = 0; i < count; i += 4){
		...
		if (id->getValue() == gameKind){
			...
			szLotNum->retain();
			szShijian->retain();
			if (id->getValue() == CZ_SEVENSTAR)
			{
				memset(CQiXingCaiRule::m_lastExpect, 0, KJ_QIHAO_LENGTH);
				memset(CQiXingCaiRule::m_lastKjShj, 0, KJ_SHIJIAN_LENGTH);
				string strExpect = mPeriod->getCString();
				if (strExpect.substr(0, 2) != "20")
					strExpect = "20" + strExpect;
				memcpy(CQiXingCaiRule::m_lastExpect, strExpect.c_str(), strExpect.length());
				memcpy(CQiXingCaiRule::m_lastKjShj, szShijian->getCString(), strlen(szShijian->getCString()));
			}
			//this->updateLotteryId();
			//this->updateCurrentTime();
			break;
...
void LotteryGambleLayer::updateLotteryId()
{
...
void LotteryGambleLayer::updateCurrentTime()
{
	String *headStr = NULL;
	m_bIsFendan = gameRule->IsFenDanDuration();
...
void LotteryGambleLayer::touZhuRet(Object *obj)
{
...
//top menu
void LotteryGambleLayer::setting(Object* obj)
{
	playButtonSound();
...
void LotteryGambleLayer::jumpResult(Object* obj)
{	
	playButtonSound();	
...
void LotteryGambleLayer::XuanZeMonenyLayer(Object *obj)
{
	//playButtonSound();
...
//middle menu
void LotteryGambleLayer::toMore(Object* obj)
{
	playButtonSound();
...
void LotteryGambleLayer::toRandomGamble(Object* obj)
{
	playButtonSound();
...
void LotteryGambleLayer::ZhuiHaoTouZhuJinENumber(Object *obj)
{
...
void LotteryGambleLayer::ShiYiXuanWoNumber(Object *obj)
{
...
//bottom menu
void LotteryGambleLayer::toDelete(Object* obj)
{
	playButtonSound();
...
void LotteryGambleLayer::toCart(Object* obj)
{
	playButtonSound();
	...
	if(!bRet)
	{
		...
		Integer* b_wanFaKind = Integer::create(BetLimit::GetWanfaIdFromId(wanFaKind,4));
...
void LotteryGambleLayer::toGamble(Object* obj)
{
	playButtonSound();
	...
	if(!bRet)
	{
		...
		//判断注数限制
		int maxCount = BetLimit::GetMaxBetFromId(gameKind,BetLimit::GetWanfaIdFromId(wanFaKind,4));
		int nowTouzhu = betZhuShu;
		if(maxCount < nowTouzhu)
		{
			std::string strDisplay =
				BetLimit::GetWanFaNameFromId(gameKind,BetLimit::GetWanfaIdFromId(wanFaKind,4)) +
				ConfigMgr::instance()->text("display_text.xml", "t954") +
				String::createWithFormat("%d",maxCount)->getCString() +
				ConfigMgr::instance()->text("display_text.xml", "t0");
		...
		if(!BetLimit::CheckDingWeiDanStr(BetLimit::GetWanfaIdFromId(wanFaKind,4),betNumber->getCString()))
		{
			MovingLabelLayer* layer = MovingLabelLayer::MovingLabelLayerWith(ConfigMgr::instance()->text("display_text.xml", "t184"),
				Vec2(winSize.width * 0.5,winSize.height * 0.5));
			this->addChild(layer, 255);
		...
		EntityMgr::instance()->getDispatch()->SendPacketWithTouZhuCQSSC(qishuStr, gameKind, BetLimit::GetWanfaIdFromId(wanFaKind, 4),
...
//更改玩法ID
void LotteryGambleLayer::changeKind(Object *obj)
{
	selectedItemTag = ((Integer *)obj)->getValue(); 
	//保存
	MyBetData::getInstance()->setLastWanfaId(tagQiXingCai[selectedItemTag]);
	//
	...
	this->setWanFaKind(tagQiXingCai[selectedItemTag]);
	betArea->clearNumbers();
	betArea->cancelAllMenuSelect();
	betZhuShuTouZhuJinE = 0;
...
void LotteryGambleLayer::deleteItem(Object *obj)
{
...
void LotteryGambleLayer::addRandomItem(Object *obj)
{
	this->setKeypadEnabled(true);
	betZhuShuTouZhuJinE = 0;
	beilvInput->setText("1");
...
void LotteryGambleLayer::cartConfirm(Object *obj)
{	
...
void LotteryGambleLayer::clearNumbers(Object *obj)
{
	//其它情况，清空
	MyBetNumber::getInstance()->clear();
...
void LotteryGambleLayer::getSystemTimeObject(Object *obj)
{
...
void LotteryGambleLayer::calculateCQSSCQiShu()
{

}

void LotteryGambleLayer::onEnter()
{
	BaseLayer::onEnter();
}

void LotteryGambleLayer::onExit()
{
	BaseLayer::onExit();
}

void LotteryGambleLayer::back(Object *obj)
{
	playButtonSound();
...
void LotteryGambleLayer::onKeyReleased(EventKeyboard::KeyCode keycode, Event *event)
{
...
void LotteryGambleLayer::showKindView()
{
...
void LotteryGambleLayer::hideKindView()
{
	LayerColor* layer = (LayerColor *)this->getChildByTag(9527);
	if(layer == nullptr) return;
...
void LotteryGambleLayer::updateTouZhuJine(Object *obj)
{
	betArea->getBetNumber();
	betZhuShuTouZhuJinE = betArea->getBetZhuShu();
...
void LotteryGambleLayer::GetLastYueRet(Object *obj)
{
...
//------------------------------------------------新增----------------------------------------------------------------
void LotteryGambleLayer::editBoxEditingDidEnd(EditBox* editBox)
{
...
void LotteryGambleLayer::FanDianMinus(Object *obj)
{
	playButtonSound();
	...
	listener->onTouchBegan = CC_CALLBACK_2(LotteryGambleLayer::onTouchBegan,this);
	listener->onTouchEnded = CC_CALLBACK_2(LotteryGambleLayer::onTouchEnded,this);
	listener->onTouchMoved = CC_CALLBACK_2(LotteryGambleLayer::onTouchMoved,this);
	...
	MenuItemSprite* MakeSureMenuItem = MenuItemSprite::create(MakeSureSprite_1,MakeSureSprite_2,CC_CALLBACK_1(LotteryGambleLayer::SuccenGameResume,this));
	MenuItemSprite* PCloseItemMenuItem = MenuItemSprite::create(PCloseItem_1,PCloseItem_2,CC_CALLBACK_1(LotteryGambleLayer::pCoseGameResume,this));
	...
	MenuItemSprite* MinusMenuItem = MenuItemSprite::create(MinusValueSprite_1,MinusValueSprite_2,CC_CALLBACK_1(LotteryGambleLayer::PlusFianDian,this));
	MinusMenuItem->setAnchorPoint(Vec2(0.5,0.5));
	MinusMenuItem->setPosition(Vec2(winSize.width*0.84 + 10,winSize.height*0.5 + tempInt));
	MenuItemSprite* PlusMenuItem = MenuItemSprite::create(PlusValueSprite_1,PlusValueSprite_2,CC_CALLBACK_1(LotteryGambleLayer::MinusFanDian,this));
...
void LotteryGambleLayer::PlusFianDian(Object *obj)
{
...
void LotteryGambleLayer::MinusFanDian(Object *obj)
{
	float PlusFanDianvalie =PlusFanDianvalieMAX;
...
void LotteryGambleLayer::pCoseGameResume(Object *obj)
{
...
void LotteryGambleLayer::SuccenGameResume(Object *obj)
{
	playButtonSound();
	...
	if(!bRet)
	{
		...
		EntityMgr::instance()->getDispatch()->SendPacketWithGetTouZhuSetUserBonus(gameKind, BetLimit::GetWanfaIdFromId(wanFaKind,4), PlusFanDianvalieMAX, BaiFenbiNumber);
	}
}

/**触摸开始*/
bool LotteryGambleLayer::onTouchBegan(Touch *pTouch, Event *pEvent)
{	
...
void LotteryGambleLayer::onTouchMoved(Touch *pTouch, Event *pEvent)
{

}

void LotteryGambleLayer::onTouchEnded(Touch *pTouch, Event *pEvent)
{	

}

void LotteryGambleLayer::KindMyFanDianRet(Object *obj)
{
	...
	int count = data->count();
	if (count == 0) return;
	int myWanFa = BetLimit::GetWanfaIdFromId(wanFaKind,4);
	for (int i = 0;i < count; i += 4)
	{
		Integer *id = (Integer*) data->objectAtIndex(i);
...
void LotteryGambleLayer::isFenDanHour(Object *obj)
{
	NotificationCenter::getInstance()->postNotification("IsFendanHourRet", Bool::create(m_bIsFendan));
}

void LotteryGambleLayer::kuaNianExpect(Object *obj)
{
	Dictionary* data = (Dictionary *)obj;
...
void LotteryGambleLayer::pressMinus(Object *obj)
{
...
void LotteryGambleLayer::beiAdd(Object *obj)
{
	playButtonSound();
...
void LotteryGambleLayer::beiMinus(Object *obj)
{
	playButtonSound();
...
void LotteryGambleLayer::updateMoneyLabel()
{
...
void LotteryGambleLayer::tableCellTouched(TableView* table, TableViewCell* cell)
{
...
cocos2d::Size LotteryGambleLayer::cellSizeForTable(TableView *table)
{
	cocos2d::Size result;
...
TableViewCell* LotteryGambleLayer::tableCellAtIndex(TableView *table, ssize_t idx)
{
	...
	if(table->getTag() == 1000) //菜单
	{
		...
		const char* tempStr = ConfigMgr::instance()->text("display_7xing.xml",String::createWithFormat("t%d",20 +  tagQiXingCai[nowPos])->getCString());
...
ssize_t LotteryGambleLayer::numberOfCellsInTableView(TableView *table)
{
	if(table->getTag() == 1000) //菜单
	{
...
void LotteryGambleLayer::scrollViewDidScroll(ScrollView* view)
{

}

void LotteryGambleLayer::scrollViewDidZoom(ScrollView* view)
{

}

void LotteryGambleLayer::transferString(string &str, int nStep)
{
	bool bTeMa = false;
	int nPos = str.find("|");
	if (nPos >= 0)
...
void LotteryGambleLayer::queryGameMoreRet(Object *obj)
{
