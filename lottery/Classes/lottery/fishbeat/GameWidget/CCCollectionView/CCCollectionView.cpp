...
{
    return CollectionView::create(nullptr, nullptr, cocos2d::Size::ZERO);
}

CollectionView* CollectionView::create(CollectionViewDataSource* dataSource, CollectionViewDelegate* delegate, cocos2d::Size size)
{
    return CollectionView::create(dataSource, delegate, size, NULL);
}

CollectionView* CollectionView::create(CollectionViewDataSource* dataSource, CollectionViewDelegate* delegate, cocos2d::Size size, Node *container)
{
...
bool CollectionView::initWithViewSize(cocos2d::Size size, Node* container/* = NULL*/)
{
...
void CollectionView::setBackGroundViewWithFile(const char *fileName)
{
    if(_backgroundView)
    {
        ...
        cocos2d::Size size = texture2D->getContentSize();
        _backgroundView->setTextureRect(Rect(0, 0, size.width, size.height));
    }
    else
    {
        _backgroundView = Sprite::create(fileName);
        Vec2 position = Vec2(this->getViewSize().width*0.5f, this->getViewSize().height*0.5f);
        _backgroundView->cocos2d::Node::setPosition(position);
        _backgroundView->setAnchorPoint(Vec2(0.5,0.5));
...
bool CollectionView::onTouchBegan(Touch *pTouch, Event *pEvent)
{
    ...
    {
		_touchedCell->onCellUnhighlight();
        if(_delegate != NULL)
        {			
...
void CollectionView::_updateCellPositions()
{
    ...
    _vCellsPositions.resize(cellsCount + 1, Vec2(0,0));
    ...
    if (cellsCount > 0)
    {
        ...
        for (int i=0; i < cellsCount+1; i++)
        {
            ...
            _vCellsPositions[i] = Vec2(xOffset, yOffset);
        }
...
void CollectionView::_updateContentSize()
{
    ...
    {
        float maxPosition = 0;
        cocos2d::Size cellSize = _delegate->collectionCellSizeForIndex(this, 0);
        ...
        size = cocos2d::Size(_viewSize.width, maxPosition);
    }
...
Vec2 CollectionView::_offsetFromIndex(ssize_t index)
{
    Vec2 offset = this->__offsetFromIndex(index);
    
    const cocos2d::Size cellSize = _delegate->collectionCellSizeForIndex(this, index);
    offset.y = this->getContainer()->getContentSize().height - offset.y - cellSize.height;
    return offset;
}

Vec2 CollectionView::__offsetFromIndex(ssize_t index)
{
    Vec2 offset;
    cocos2d::Size  cellSize;
    offset = Vec2(_vCellsPositions[index].x, _vCellsPositions[index].y);
    return offset;
}

long CollectionView::_indexFromOffset(Vec2 offset)
{
    long index = 0;
...
long CollectionView::__indexFromOffset(Vec2 offset)
{
    long low = 0;
...
ssize_t CollectionView::_getIndexWithTouchPoint(Vec2 point)
{
    for (int i=0; i<_cellsUsed.size(); i++)
    {
        CollectionViewCell* cell = _cellsUsed.at(i);
        Rect rect;
		cell->getCellRect(rect);
