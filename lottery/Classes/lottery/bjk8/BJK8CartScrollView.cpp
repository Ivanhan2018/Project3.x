#include "RoomLayer.h"
#include "BJK8CartScrollView.h"
#include "LotteryGambleLayer.h"
using namespace cocos2d;

BJK8CartScrollView::BJK8CartScrollView()
{
...
Scene* BJK8CartScrollView::scene()
{
	Scene * scene = NULL;
	do 
	{
		scene = Scene::create();
		CC_BREAK_IF(! scene);
		BJK8CartScrollView *layer = BJK8CartScrollView::create();
		CC_BREAK_IF(! layer);
		scene->addChild(layer);
	} while (0);
	return scene;
}
bool BJK8CartScrollView::init()
{
	bool bRet = false;
	do 
	{
		...
		listener->onTouchBegan = CC_CALLBACK_2(BJK8CartScrollView::onTouchBegan, this);	
		listener->onTouchEnded = CC_CALLBACK_2(BJK8CartScrollView::onTouchEnded, this);
		listener->onTouchMoved = CC_CALLBACK_2(BJK8CartScrollView::onTouchMoved, this);
		listener->onTouchCancelled = CC_CALLBACK_2(BJK8CartScrollView::onTouchCancelled, this);
		...
		gap3->setScaleX(SCREEN_WIDTH / gap1->getContentSize().width);
		...
		gap2->setScaleX(SCREEN_WIDTH / gap1->getContentSize().width);
...
void BJK8CartScrollView::pressKindButton(int tag)
{
	playButtonSound();
...
void BJK8CartScrollView::onEnter()
{
	Layer::onEnter();
	//
...
void BJK8CartScrollView::onExit()
{
	//
	Layer::onExit();
}

bool BJK8CartScrollView::onTouchBegan(Touch *pTouch, Event *pEvent)
{
...
void BJK8CartScrollView::onTouchMoved(Touch *pTouch, Event *pEvent)
{
	Vec2 touchPoint = Director::getInstance()->convertToGL(pTouch->getLocationInView());
	touchPoint = m_node->convertToNodeSpace(touchPoint);
	m_end = touchPoint.y;
...
void BJK8CartScrollView::onTouchEnded(Touch *pTouch, Event *pEvent)
{
	Vec2 touchPoint = Director::getInstance()->convertToGL(pTouch->getLocationInView());
	touchPoint = m_node->convertToNodeSpace(touchPoint);
	endPoint = touchPoint;
...
void BJK8CartScrollView::onTouchCancelled(Touch *pTouch, Event *pEvent)
{
