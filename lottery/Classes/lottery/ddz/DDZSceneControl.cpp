...
#include "RoomLayer.h"
#include "ConfigMgr.h"
DDZSceneControl* DDZSceneControl::sharedDDZSceneControl()
{
	static DDZSceneControl* 	s_DDZSceneControl=NULL;
	if (!s_DDZSceneControl)
	{
		s_DDZSceneControl=new DDZSceneControl();
	}
	return s_DDZSceneControl;
}

Scene* DDZSceneControl::getScene(int index,bool bEffect,float t)
{
	...
	case SCENE_game:
		{
			s->addChild(DDZSceneView::create());
	...
	do 
	{
		...
		case 18: return CCTransitionFade::create(t, s,Color3B::WHITE);
