#include "RoomLayer.h"
#include "FC3DPL3KindScrollView.h"
...
FC3DPL3KindScrollView::FC3DPL3KindScrollView()
{
...
Scene* FC3DPL3KindScrollView::scene()
{
	Scene * scene = NULL;
	do 
	{
		scene = Scene::create();
		CC_BREAK_IF(! scene);
		FC3DPL3KindScrollView *layer = FC3DPL3KindScrollView::create();
		CC_BREAK_IF(! layer);
		scene->addChild(layer);
	} while (0);
	return scene;
}
bool FC3DPL3KindScrollView::init()
{
	bool bRet = false;
	do 
	{
...
void FC3DPL3KindScrollView::initData()
{
	//界面数据
	lyWanFa = nullptr;
	m_node = nullptr;
...
void FC3DPL3KindScrollView::initView()
{
	auto listener = EventListenerTouchOneByOne::create();
	listener->setSwallowTouches(true);
	listener->onTouchBegan = CC_CALLBACK_2(FC3DPL3KindScrollView::onTouchBegan, this);	
	listener->onTouchEnded = CC_CALLBACK_2(FC3DPL3KindScrollView::onTouchEnded, this);
	listener->onTouchMoved = CC_CALLBACK_2(FC3DPL3KindScrollView::onTouchMoved, this);
	listener->onTouchCancelled = CC_CALLBACK_2(FC3DPL3KindScrollView::onTouchCancelled, this);
	...
	int chongQingKindCount = atoi(ConfigMgr::instance()->text("display_3D.xml", "t1000"));
	int iViewHeight = 0;
	...
	{
		//位置
		int iPosTitle = atoi(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", 1000+i + 1)->getCString()));
		//分类名称 文字
		LabelTTF *kindTitleLabel = LabelTTF::create(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", iPosTitle)->getCString()), "", fontSize);
		...
		//大分类的个数
		int cataCount = atoi(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", iPosTitle + 1)->getCString()));

		for(int cata_i = 0;cata_i < cataCount;cata_i ++)
		{
			int iPosCata = atoi(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", iPosTitle + cata_i + 2)->getCString()));

			//显示前面 分类名称
			std::string strTitle = ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", iPosCata)->getCString());
			...
			//菜单叶子 位置
			int itemPos = atoi(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", iPosCata + 1)->getCString()));

			//叶子个数
			int itemCount = atoi(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", itemPos)->getCString()));

			for(int j = 0 ; j < itemCount ; j++){

				String* kindStr = String::create(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d", itemPos + j + 1)->getCString()));
				...
				if(kindItemTag == selectedSpriteTag){
					...
					//文字
					LabelTTF *kindItem = LabelTTF::create(ConfigMgr::instance()->text("display_3D.xml",kindStr->getCString()), "", fontSize);
					kindItem->setAnchorPoint(Vec2::ANCHOR_MIDDLE);	
					kindItem->setTag(9527);
					kindItem->setPosition(sptKindItem->getContentSize() / 2);					
...
void FC3DPL3KindScrollView::pressKindButton(int tag)
{
	playButtonSound();
...
void FC3DPL3KindScrollView::onEnter()
{
	Layer::onEnter();
	//
...
void FC3DPL3KindScrollView::onExit()
{
	//
	Layer::onExit();
}

bool FC3DPL3KindScrollView::onTouchBegan(Touch *pTouch, Event *pEvent)
{
...
void FC3DPL3KindScrollView::onTouchMoved(Touch *pTouch, Event *pEvent)
{
...
void FC3DPL3KindScrollView::onTouchEnded(Touch *pTouch, Event *pEvent)
{
	selectedSprite->setVisible(false);
...
void FC3DPL3KindScrollView::onTouchCancelled(Touch *pTouch, Event *pEvent)
{

}

void FC3DPL3KindScrollView::refreshContent()
{
	...
	for(int i = 0;i < LIST_MAX_SIZE;i ++)
	{
		...
		if(!isOutList)
		{
			ltfWord->setString(ConfigMgr::instance()->text("display_3D.xml",String::createWithFormat("t%d",20 + treeNumbersMap[MyBetData::getInstance()->getWanfaAt(i)])->getCString()));
			if(m_isNowDel)
			{
...
void FC3DPL3KindScrollView::refreshMenu()
{
