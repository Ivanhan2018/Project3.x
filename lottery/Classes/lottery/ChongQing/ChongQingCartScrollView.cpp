#include "RoomLayer.h"
#include "ChongQingCartScrollView.h"
#include "ChongQingGambleLayer.h"
using namespace cocos2d;

float myabs1(float a)
{
	return a>0?a:-a;
}

ChongQingCartScrollView::ChongQingCartScrollView()
{
...
Scene* ChongQingCartScrollView::scene()
{
	Scene * scene = NULL;
	do 
	{
		scene = Scene::create();
		CC_BREAK_IF(! scene);
		ChongQingCartScrollView *layer = ChongQingCartScrollView::create();
		CC_BREAK_IF(! layer);
		scene->addChild(layer);
	} while (0);
	return scene;
}
bool ChongQingCartScrollView::init()
{
	bool bRet = false;
	do 
	{
		...
		listener->onTouchBegan = CC_CALLBACK_2(ChongQingCartScrollView::onTouchBegan, this);	
		listener->onTouchEnded = CC_CALLBACK_2(ChongQingCartScrollView::onTouchEnded, this);
		listener->onTouchMoved = CC_CALLBACK_2(ChongQingCartScrollView::onTouchMoved, this);
		listener->onTouchCancelled = CC_CALLBACK_2(ChongQingCartScrollView::onTouchCancelled, this);
...
void ChongQingCartScrollView::pressKindButton(int tag)
{
	playButtonSound();
...
void ChongQingCartScrollView::onEnter()
{
	Layer::onEnter();
	//
...
void ChongQingCartScrollView::onExit()
{
	//
	Layer::onExit();
}

bool ChongQingCartScrollView::onTouchBegan(Touch *pTouch, Event *pEvent)
{
...
void ChongQingCartScrollView::onTouchMoved(Touch *pTouch, Event *pEvent)
{
	Vec2 touchPoint = Director::getInstance()->convertToGL(pTouch->getLocationInView());
	touchPoint = m_node->convertToNodeSpace(touchPoint);
	m_end = touchPoint.y;
...
void ChongQingCartScrollView::onTouchEnded(Touch *pTouch, Event *pEvent)
{
	Vec2 touchPoint = Director::getInstance()->convertToGL(pTouch->getLocationInView());
	touchPoint = m_node->convertToNodeSpace(touchPoint);
	endPoint = touchPoint;
	...
	if (myabs1(startPoint.x - endPoint.x) > DELTA ||   
		myabs1(startPoint.y - endPoint.y) > DELTA)   
	{ 
		selectedSpriteTag = INVALID_TAG;
		return;  
	}
...
void ChongQingCartScrollView::onTouchCancelled(Touch *pTouch, Event *pEvent)
{
