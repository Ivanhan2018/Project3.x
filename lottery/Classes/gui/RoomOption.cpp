...
bool RoomOption::init()
{
	...
	{
		pPlayerAccountTips = LabelTTF::create(ConfigMgr::instance()->text("t131"),"",22);
	}
	else
	{
		pPlayerAccountTips = LabelTTF::create(ConfigMgr::instance()->text("t309"),"",22);
	}
	...
	//声音
	LabelTTF* pVoiceTipsLabel = LabelTTF::create(ConfigMgr::instance()->text("t132"),"",25);
	...
	LabelTTF* pSilentTipsLabel  = LabelTTF::create(ConfigMgr::instance()->text("t133"),"",22);
	...
	LabelTTF* pMusicTipsLabel  = LabelTTF::create(ConfigMgr::instance()->text("t134"),"",22);
	...
	LabelTTF* pSoundTipsLabel  = LabelTTF::create(ConfigMgr::instance()->text("t135"),"",22);
	...
	//其他
	LabelTTF* pOtherTipsLabel = LabelTTF::create(ConfigMgr::instance()->text("t136"),"",25);
	...
	LabelTTF* pCheckTipsLabel  = LabelTTF::create(ConfigMgr::instance()->text("t137"),"",22);
	...
	const char* pVesion = String::createWithFormat("%d.%d.%d",ConfigMgr::instance()->m_VersionInfo.m_nHighVersion,ConfigMgr::instance()->m_VersionInfo.m_nMiddleVersion,ConfigMgr::instance()->m_VersionInfo.m_nLowVersion)->getCString();
...
void RoomOption::onTouchMoved(Touch *pTouch, Event *pEvent)
{
	...
	if(m_nChangeVoice == 1)
	{
		if(touchPoint.x >= m_pMusicControlBlackBack->getPositionX() - m_pMusicControlBlackBack->getContentSize().width * 0.5 && touchPoint.x <= m_pMusicControlBlackBack->getPositionX() + m_pMusicControlBlackBack->getContentSize().width * 0.5)
		{
			...
			//此处改变音乐音量大小
			CocosDenshion::SimpleAudioEngine::getInstance()->setBackgroundMusicVolume((m_pSoundControlBlackBack->getContentSize().width * 0.5 + touchPoint.x - m_pSoundControlBlackBack->getPositionX())/m_pSoundControlBlackBack->getContentSize().width);
