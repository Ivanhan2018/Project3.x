...
void LuckyDraw::pressDraw( Object* obj)
{
	...
	canDraw = false;
    EntityMgr::instance()->getDispatch()->SendPacketWithTurnTable();
//	this->addRandSeed();
//	int num = rand()%8;
//    lastLuckyNum = num;
//	this->pinMove(num);
...
void LuckyDraw::pinMove(int tag)
{
	...
	RotateBy *rotateBy = RotateBy::create(10.0, finalAngle);
	EaseExponentialOut *easeAction = EaseExponentialOut::create(dynamic_cast<CCActionInterval *>(rotateBy));
...
void LuckyDraw::pinMoveEnd(Node *obj)
{
    ...
    promptBox->setPromptText(ConfigMgr::instance()->text(String::createWithFormat("t%d",1011+lastLuckyNum)->getCString()));
