...
bool GroupSprite::initGroupSprite(const char* str1,const char* str2,GorupSpriteType type,int level)
{
	...
	{
		const char * str = String::createWithFormat("%s%s",str2,ConfigMgr::instance()->text("t453"))->getCString();
	...
	}else if(type == GorupSpriteTypePhotoAndText1)
	{
		...
		if (a >= 10000)
		{
			if (a%10000)
			{
				snprintf(szLess, sizeof(szLess)-1, "%.1f%s", a/10000.f, ConfigMgr::instance()->text("t469"));
			}
			else
			{
				snprintf(szLess, sizeof(szLess)-1, "%d%s", a/10000, ConfigMgr::instance()->text("t469"));
			}
		}
		else
		{
		...
		if (b >= 10000)
		{
			if (b%10000)
			{
				snprintf(szMax, sizeof(szMax)-1, "%.1f%s", b/10000.f, ConfigMgr::instance()->text("t469"));
			}
			else
			{
				snprintf(szMax, sizeof(szMax)-1, "%d%s", b/10000, ConfigMgr::instance()->text("t469"));
			}
		}
		else
		{
		...
		if (0 != a && 0 != b)
		{
			snprintf(szText, sizeof(szText) - 1, "%s-%s%s", szLess, szMax, ConfigMgr::instance()->text("t470"));	// a-b以下
		}
		else if (0 == b)
		{
			snprintf(szText, sizeof(szText) - 1, "%s%s", szLess, ConfigMgr::instance()->text("t471"));	// a以上
		}
		else if (0 == a)
		{
			snprintf(szText, sizeof(szText) - 1, "%s%s", szMax, ConfigMgr::instance()->text("t470"));	// b以下
		}
		else
		{
			snprintf(szText, sizeof(szText) - 1, "0%s", ConfigMgr::instance()->text("t471"));	// 0以上
		}
		...
		if(level == 1)
		{
			...
			RotateBy* action1 = RotateBy::create(0.8f,-10);
			RotateBy* action2 = RotateBy::create(0.8f,10);
		...
		if(level == 3)
		{
			...
			RotateBy* action1 = RotateBy::create(0.9f,-10);
			RotateBy* action2 = RotateBy::create(0.9f,10);
