...
bool FreeCell::initFreeCellWith(int type,bool isGet){
		...
		if(!isGet)
		{
			pTextWaring = LabelTTF::create(ConfigMgr::instance()->text("t170"),"",22);
		}
		else
		{
			pTextWaring = LabelTTF::create(ConfigMgr::instance()->text("t459"),"",22);
		}
		...
		//const char *str = String::createWithFormat("%s%d%s",ConfigMgr::instance()->text("t452"),0,"%")->getCString();
	...
	case 0:
		{	
			...
			m_text->setString(ConfigMgr::instance()->text("t175"));	
 			//pTTF->setString(String::createWithFormat("%s%d%s",ConfigMgr::instance()->text("t452"),m_csmj_per,"%")->getCString());	
			break;
		}
	case 1:
		{
			...
			m_text->setString(ConfigMgr::instance()->text("t178"));	
 		    //pTTF->setString(String::createWithFormat("%s%d%s",ConfigMgr::instance()->text("t452"),m_pdk_per,"%")->getCString());
		break;
		  }
	case 2:
		{
			...
			m_text->setString(ConfigMgr::instance()->text("t177"));	
 			//pTTF->setString(String::createWithFormat("%s%d%s",ConfigMgr::instance()->text("t452"),m_phz_per,"%")->getCString());
			break;
		}
	...
	case 3:
		{
			...
			m_text->setString(ConfigMgr::instance()->text("t176"));	
 			//pTTF->setString(String::createWithFormat("%s%d%s",ConfigMgr::instance()->text("t452"),m_sdh_per,"%")->getCString());
			break;
		}
