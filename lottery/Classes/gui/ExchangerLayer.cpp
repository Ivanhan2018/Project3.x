...
//        buyArticleNum->setText("1");
		...
		//联系方式标签
		textWidth = 100;
		contactLabel = LabelTTF::create(ConfigMgr::instance()->text("t1000"),"",30,cocos2d::Size(textWidth,0),kCCTextAlignmentLeft);
...
void ExchangerLayer::pressExchangeButton(Object *obj)
{
	...
	for(int i = 0 ; i < itemNum ; i++){
		...
		{
			buyArticleType2->setString(ConfigMgr::instance()->text(String::createWithFormat("t%d", 589+i)->getCString()));
		}
		if(currentType<3){
...
//    string str1 = buyArticleNum->getText();
    ...
    if(str1!=""&&str2!="")
    {
        if(this->isNumber(str1.c_str()) == false){
            ...
            promptBox->setPromptText(ConfigMgr::instance()->text("t1007"));
           this->addChild(promptBox,3);
        }else if(this->isNumber(str2.c_str()) == false){
            promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
            promptBox->setPromptText(ConfigMgr::instance()->text("t1008"));
           this->addChild(promptBox,3);
        }else{
        ...
        promptBox = PromptBox::PromptBoxWith(Vec2(winSize.width * 0.5,winSize.height * 0.5),mPromptTypeGetError);
        promptBox->setPromptText(ConfigMgr::instance()->text("t1005"));
       this->addChild(promptBox,3);
    }
}
