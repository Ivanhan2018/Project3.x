...
{
    EntityMgr::instance()->getDispatch()->SendPacketWithGetUserPacket();
}
...
void PlayerInfoLayer::onModifyTouxiangResult(Object* obj)
{
	...
	if(i == 0)
	{
		...
		MovingLabelLayer* layer = MovingLabelLayer::MovingLabelLayerWith(ConfigMgr::instance()->text("t408"),Vec2(427,240));
		this->addChild(layer,20);
		//�ɹ�
	}
	else
	{
		MovingLabelLayer* layer = MovingLabelLayer::MovingLabelLayerWith(ConfigMgr::instance()->text("t162"),Vec2(427,240));
		this->addChild(layer);
		//ʧ��
	}
}
...
void PlayerInfoLayer::onModifyNickNameResult(Object* obj)
{
	...
	if(i == 0)
	{
		...
		{
			m_pUserName->setString(String::createWithFormat("%s%s",ConfigMgr::instance()->text("t82"),m_szModifyNickName)->getCString());
		}
		else
		{
			m_pUserName->setString(String::createWithFormat("%s%s",ConfigMgr::instance()->text("t82"),m_szModifyNickName)->getCString());
		...
		pMenu1 = NULL;
		MovingLabelLayer* layer = MovingLabelLayer::MovingLabelLayerWith(ConfigMgr::instance()->text("t409"),Vec2(427,240));
		this->addChild(layer,20);
		//�ɹ�
	}
	else
	...
	if(tableTag == 0){
		...
		if(cell == NULL)
		{
            ...
            if(rand()%2==0){
                ...
                sprintf(name, "%s%d.png", SPRITE_MEDAL_L, (int)index*MEDAL_TABLE_COLUMN+i);
            }
		...
		{
			sprintf(name, "article%d.png", (int)index*ARTICLE_TABLE_COLUMN+i);
			...
			String *str = String::createWithFormat("%s %d %s", 
				ConfigMgr::instance()->text(String::createWithFormat("t%d",2010+(int)index*ARTICLE_TABLE_COLUMN+i)->getCString())
				, article_num[(int)index*ARTICLE_TABLE_COLUMN+i],
				ConfigMgr::instance()->text(String::createWithFormat("t%d",2020+(int)index*ARTICLE_TABLE_COLUMN+i)->getCString()));
...
void PlayerInfoLayer::showMedalInfo(Object *obj, Control::EventType event)
{
	...
	{
		Color3B textColor = ccc3(0,255,0);
		labelMedalInfo->setString(ConfigMgr::instance()->text(String::createWithFormat("t%d", 720+tag)->getCString()));
		labelMedalInfo->setColor(textColor);
		...
		//FiniteTimeAction *sequence = Sequence::create(Show::create(), tintTo, Hide::create(), NULL);
		FiniteTimeAction *sequence = Sequence::create(Show::create(), DelayTime::create(1.5f), Hide::create(), NULL);
