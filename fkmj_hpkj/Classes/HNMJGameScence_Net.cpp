...
void HNMJGameScence::initNet()
{
	...
	addNetCBDefine(SUB_S_TIP_INFO,this,HNMJGameScence::OnSubTipInfo);
	...
	addNetCBDefine(SUB_S_SEND_RECORD,this,HNMJGameScence::OnSubSendRecord);///
...
//
void HNMJGameScence::OnSubTipInfo( const void *pBuffer, WORD wDataSize )
{
	//效验消息
	ASSERT(wDataSize==sizeof(CMD_S_TipInfo));
	if (wDataSize!=sizeof(CMD_S_TipInfo)) 
		return;
...
void HNMJGameScence::OnSubSendRecord(const void *pBuffer, WORD wDataSize)
{
	...
	//}
//消息处理
	utility::log("HNMJGameScence::OnSubSendRecord wDataSize=%d,sizeof(tagServerGameRecord)=%d",wDataSize,sizeof(tagServerGameRecord));
	//效验大小 
	...
	for (WORD wIndex= 0; wIndex<temp; wIndex++)
	{
		...
		utility::log("wIndex=%d,temp=%d,m_cbPlayerCount=%d,m_cbBankerCount=%d",wIndex,temp,m_cbPlayerCount,m_cbBankerCount);
	}
	return;
}
