...
bool CSocketEngine::connect(const char* url, int port)
{
	...
	cocos2d::log("Connect %s:%d",url,port);
...
bool CSocketEngine::send(int main, int sub, void* data, int dataSize)
{
	...
	//加密数据
	WORD wSendSize = sizeof(TCP_Head) + dataSize;
	bool bRet=Common_MappedBuffer(cbDataBuffer, sizeof(TCP_Head) + dataSize);
	mSocket.send((const char*)cbDataBuffer, wSendSize);
...
void CSocketEngine::sendTCPValidate()
{
	...
	char16_t szValidateKey[64]={0};
	std::u16string strUTF16;
	bool bRet1=cocos2d::StringUtils::UTF8ToUTF16(szCompilatio,strUTF16);
	std::vector<char16_t> v1=cocos2d::StringUtils::getChar16VectorFromUTF16String(strUTF16);
	memcpy(szValidateKey,&v1[0],v1.size()*2);
	send(MDM_KN_COMMAND, SUB_KN_VALIDATE_SOCKET,(void *)szValidateKey,sizeof(szValidateKey));
	//mSocket.send((const char *)szValidateKey[0],128);
}
