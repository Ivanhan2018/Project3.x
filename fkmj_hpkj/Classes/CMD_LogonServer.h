...
#define SUB_GP_LIST_SERVER			102//104									//房间列表
#define SUB_GP_LIST_MATCH			105									//比赛列表
#define SUB_GP_VIDEO_OPTION			106									//视频配置
	
//完成信息
#define SUB_GP_LIST_FINISH			103//200									//发送完成
...
struct CMD_GP_LogonAccounts
{
	...
	char16_t							szMachineID[LEN_MACHINE_ID];		//机器序列

	char16_t							szPassword[LEN_MD5];				//登录密码
	char16_t							szAccounts[LEN_ACCOUNTS];			//登录帐号
...
struct CMD_GP_LogonSuccess
{
	...
	//用户成绩
	double							lUserScore;				            //用户积分
	double							lUserInsure;						//用户银行
	double                          dwUserMedal;                        //用户奖牌 

	//用户信息
	byte							cbGender;							//用户性别
	byte							cbMoorMachine;						//锁定机器
	char16_t						szAccounts[LEN_ACCOUNTS];			//登录帐号
	char16_t						szNickName[LEN_ACCOUNTS];			//用户昵称
	char16_t                        szAddrDescribe[128];  //地址描述
