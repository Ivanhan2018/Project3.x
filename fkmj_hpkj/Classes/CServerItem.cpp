...
bool CServerItem::SetServerAttribute(CGameServerItem * pGameServerItem,word wAVServerPort,dword dwAVServerAddr)
{
	...
	std::string strUTF8;
	std::u16string strUTF16=mGameServer.szServerAddr;
	bool bret=cocos2d::StringUtils::UTF16ToUTF8(strUTF16,strUTF8);
	strncpy(mServerAttribute.szServerName, strUTF8.c_str(), strUTF8.size()/*LEN_SERVER*/);
...
bool CServerItem::ConnectServer(CGameServerItem * pGameServerItem,word wAVServerPort,dword dwAVServerAddr)
{
	...
	bool bret=cocos2d::StringUtils::UTF16ToUTF8(strUTF16,strUTF8);
	//创建组件
	PLAZZ_PRINTF("Connect %s",strUTF8.c_str());
	if (!mSocketEngine->connect(strUTF8.c_str(), mGameServer.wServerPort))
	{
		return false;
	}
...
bool CServerItem::SendLogonPacket()
{
	...
	//游戏版本
	LogonUserID.dwProcessVersion=PROCESS_VERSION(6,0,3);//DF::shared()->GetGameVersion();

	///LogonUserID.dwPlazaVersion=DF::shared()->GetPlazaVersion();
	std::string kPassword = "DC483E80A7A0BD9EF71D8CF973673924";
	std::string kMachineID="M123";
	std::u16string kPassword1,kMachineID1;
	...
	memcpy(LogonUserID.szPassword,&v2[0],v2.size()*2);
	std::vector<char16_t> v3=cocos2d::StringUtils::getChar16VectorFromUTF16String(kMachineID1);
	memcpy(LogonUserID.szMachineID,&v3[0],v3.size()*2);
	//登录信息
	LogonUserID.dwUserID=pGlobalUserData->dwUserID;
	///memcpy(LogonUserID.szPassword,pGlobalUserData->szPassword,countarray(LogonUserID.szPassword));
	LogonUserID.wBehaviorFlags	= 0x1000|0x0002;				// 行为标识
	LogonUserID.wPageTableCount	= 6;	

	//发送数据
	SendSocketData(MDM_GR_LOGON,SUB_GR_LOGON_MOBILE/*SUB_GR_LOGON_USERID*/,&LogonUserID,sizeof(LogonUserID));
	return true;
}
