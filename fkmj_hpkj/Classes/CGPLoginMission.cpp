...
bool CGPLoginMission::onSocketSubLogonSuccess(void* data, int size)
{
	...
	std::string strUTF8NickName;
	bool bRet=cocos2d::StringUtils::UTF16ToUTF8(pData->szNickName,strUTF8NickName);
	strcpy(pGlobalUserData->szNickName, strUTF8NickName.c_str());
...
bool CGPLoginMission::onSocketListKind(void* data, int size)
{
	...
	//ASSERT(size%itemSize==0);//2940%82
