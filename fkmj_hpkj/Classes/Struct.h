...
struct tagGameServer
{
	...
	//积分信息
	double							lCellScore;						//单元积分
	double							lEnterScore;					//需要积分

	//房间信息
	char16_t						szServerAddr[32];					//房间地址
	char16_t						szServerName[LEN_SERVER];			//房间名称

	//梦成网络房间密码
	//word							wServerKind;						//是否启用密码
	word							wServerType;						//房间类型
	char16_t						szServerPassWork[LEN_SERVER];		//房间密码
#else
	word							wKindID;							//名称索引
	word							wNodeID;							//节点索引
	word							wSortID;							//排序索引
